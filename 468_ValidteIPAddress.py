...
class Solution(object):
    ...
    def isValidIP4Seg(self, seg):
        if len(seg) > 3 or len(seg) == 0:
            return False
        for char in seg:
            if ord(char) > 57 or ord(char) < 48:
                return False
        if seg[0] == '0' or int(seg) > 255:
            return False
        return True

    def isValidIP6Seg(self, seg):
